"""Add weblinks permissions

Revision ID: add_weblinks_permissions
Revises: add_reports_permissions
Create Date: 2024-01-02 17:00:00.000000
...
revision = 'add_weblinks_permissions'
...
def upgrade():
    ...
    try:
        ...
        # Define weblinks permissions
        weblinks_permissions = [
        ...
        # Add permissions
        for perm_name, description, actions_needed in weblinks_permissions:
            # Create permission
            session.execute(
                sa.text(
        ...
        session.rollback()
        raise Exception(f"Error adding weblinks permissions: {str(e)}")
...
def downgrade():
    ...
    try:
        ...
        permission_ids = session.execute(
            ...
            {'names': tuple(weblinks_permission_names)}
        ).fetchall()
        ...
        session.rollback()
        raise Exception(f"Error removing weblinks permissions: {str(e)}")
