"""Add handoffs permissions

Revision ID: add_handoffs_permissions
Revises: add_documents_permissions
Create Date: 2024-01-02 14:00:00.000000
...
revision = 'add_handoffs_permissions'
down_revision = 'add_documents_permissions'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    try:
        ...
        # Define handoffs permissions
        handoffs_permissions = [
            ('handoffs_access', 'View handoffs', ['read']),
            ('handoffs_create', 'Create new handoffs', ['write']),
            ('handoffs_close', 'Close handoffs', ['write']),
            ('handoffs_metrics', 'View handoff metrics and statistics', ['read'])
        ]
        
        # Add permissions
        for perm_name, description, actions_needed in handoffs_permissions:
            # Create permission
            session.execute(
                sa.text(
        ...
        session.rollback()
        raise Exception(f"Error adding handoffs permissions: {str(e)}")
...
def downgrade():
    ...
    try:
        ...
        permission_ids = session.execute(
            ...
            {'names': tuple(handoffs_permission_names)}
        ).fetchall()
        ...
        session.rollback()
        raise Exception(f"Error removing handoffs permissions: {str(e)}")
