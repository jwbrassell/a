...
class Project(db.Model):
    ...
    lead = db.relationship('User', foreign_keys=[lead_id], lazy='joined')
    watchers = db.relationship('User', secondary=project_watchers, lazy='joined',
                             backref=db.backref('watched_projects', lazy='dynamic'))
    stakeholders = db.relationship('User', secondary=project_stakeholders, lazy='joined',
                                 backref=db.backref('stakeholder_projects', lazy='dynamic'))
    shareholders = db.relationship('User', secondary=project_shareholders, lazy='joined',
                                 backref=db.backref('shareholder_projects', lazy='dynamic'))
    roles = db.relationship('Role', secondary=project_roles, lazy='joined',
    ...
    comments = db.relationship('Comment', backref='project', lazy='dynamic', 
                             cascade='all, delete-orphan')
    history = db.relationship('History', backref='project', lazy='dynamic', 
                            cascade='all, delete-orphan')
...
class Task(db.Model):
    ...
    status = db.relationship('ProjectStatus', foreign_keys=[status_id], lazy='joined')
    priority = db.relationship('ProjectPriority', foreign_keys=[priority_id], lazy='joined')
    ...
    comments = db.relationship('Comment', backref='task', lazy='dynamic',
                             cascade='all, delete-orphan')
    history = db.relationship('History', backref='task', lazy='dynamic',
                            cascade='all, delete-orphan')
...
class History(db.Model):
    ...
    details = db.Column(JSON)  # Store changes in JSON format
    created_by = db.Column(db.String(100), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
