...
class Document(db.Model):
    ...
    title = db.Column(db.String(256), nullable=False, index=True)  # Added index
    content = db.Column(db.Text, nullable=False)  # HTML content from TinyMCE
    category_id = db.Column(db.Integer, db.ForeignKey('document_categories.id'), nullable=False, index=True)  # Added index
    ...
    is_template = db.Column(db.Boolean, default=False)  # New field for templates
    template_name = db.Column(db.String(256))  # New field for template name
    is_private = db.Column(db.Boolean, default=False)  # New field for privacy
